                                ...
                                <tbody>
                                    @foreach ($pendingSchedules as $schedule)
                                        <tr>
                                            ...
                                            <td> {{$schedule->contactSchedules->count()}} <a href=" {{route('schedule.approve',$schedule->id)}} "> <i class="fas fa-users"></i></a> </td>
                                            <td>{{ $schedule->status }}</td>
                                            <td>{{ $schedule->desc }}</td>
                                            <td> {{ $schedule->created_at->diffForHumans() }} </td>
                                            <td>
                                                <a href=" {{ route('schedule.reject', $schedule->id) }} "
                                                    class="btn btn-outline-info" s>View <i
                                                        class="ion ion-md-arrow-round-forward"></i></a>
                                                        <a href=" {{ route('schedule.approve', $schedule->id) }} "
                                                            class="btn btn-outline-info" s>View <i
                                                                class="ion ion-md-arrow-round-forward"></i></a>
                                            </td>
                                        </tr>
